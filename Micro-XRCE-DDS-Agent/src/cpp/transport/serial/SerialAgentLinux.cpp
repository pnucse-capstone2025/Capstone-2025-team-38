...
namespace eprosima
{
    namespace uxr
    {
        ...
        SerialAgent::SerialAgent(
            ...
            : Server<SerialEndPoint>{middleware_kind}, addr_{addr}, poll_fd_{}, buffer_{0}, framing_io_(
                                                                                                addr,
            ...
            TransportRc &transport_rc)
        {
            ...
            ssize_t bytes_written =
                framing_io_.write_framed_msg(
                    ...
                    transport_rc);
                    // printf(" send_message buffer_: ");
                    // for (ssize_t i = 0; i < bytes_written; ++i)
                    // {
                    //     printf("%02X ", static_cast<unsigned char>(output_packet.message->get_buf()[i]));
                    // }
                    // printf("\n");
