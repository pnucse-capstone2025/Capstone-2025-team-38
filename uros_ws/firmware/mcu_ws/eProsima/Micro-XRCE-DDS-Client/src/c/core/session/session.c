        ...
        size_t list_size)
{
    ...
    // 2. 버퍼에 쌓인 메시지 즉시 전송
    uxr_flash_output_streams_with_token(session);
    ...
    // 5. 타임아웃 설정 및 통신 루프 시작
    int64_t start_timestamp = uxr_millis();
    int remaining_time = timeout_ms;

    do
    {
        // 6. Agent로부터 메시지 수신 대기
        listen_message_reliably(session, remaining_time);
        
        // 7. 모든 요청에 대한 상태를 받았는지 확인
    ...
    uxrSession* session)
{
    ...
    for (uint8_t i = 0; i < session->streams.output_reliable_size; ++i)
    {
        ...
        while (uxr_prepare_next_reliable_buffer_to_send(stream, &buffer, &length, &seq_num))
        {
            ...
            // for (int i = 0; i < 4; i++)
            // {
            //     buffer[length + i] = ((uint8_t *)(session->info.nonce))[i];
            // }
            extern uint8_t hash_result[DICE_CDI_SIZE];
...
bool listen_message_reliably(
        ...
        int poll_ms)
{
    // 메시지 수신 성공 여부를 저장하는 플래그
    bool received = false;
    // poll_ms가 0보다 크면 해당 시간을, 아니면 무한대기를 위해 INT32_MAX를 총 대기 시간으로 설정
    int32_t poll = (poll_ms >= 0) ? poll_ms : INT32_MAX;
    do
    {
        // 다음 하트비트(HEARTBEAT)를 보내야 할 가장 빠른 시간. 매우 큰 값으로 초기화
        int64_t next_heartbeat_timestamp = INT64_MAX;
        // 현재 시간 기록
        int64_t timestamp = uxr_millis();
        
        // 설정된 모든 '신뢰성 있는 출력 스트림'을 순회
        for (uint8_t i = 0; i < session->streams.output_reliable_size; ++i)
        {
            uxrOutputReliableStream* stream = &session->streams.output_reliable[i];
            uxrStreamId id = uxr_stream_id(i, UXR_RELIABLE_STREAM, UXR_OUTPUT_STREAM);

            // 멀티스레드 환경에서 해당 스트림에 대한 동시 접근을 막기 위해 락을 설정
            UXR_LOCK_STREAM_ID(session, id);
            // 현재 시간에 하트비트를 보내야 하는지 확인하고, 필요하다면 보냄
            // 하트비트는 Agent로부터 메시지 수신 확인(ACKNACK)이 없을 때 주기적으로 보내 연결 상태를 확인
            if (uxr_update_output_stream_heartbeat_timestamp(stream, timestamp))
            {
                write_submessage_heartbeat(session, id);
            }

            // 모든 스트림 중에서 다음 하트비트 시간이 가장 이른 시간을 기록
        ...
        {
            // 대기 시간이 0이면 최소 1ms는 대기하도록 설정
            poll_to_next_heartbeat = 1;
        }

        // 이번 루프에서 실제로 메시지를 기다릴 시간 결정
        // (전체 남은 시간과 다음 하트비트까지 남은 시간 중 더 짧은 시간)
        int poll_chosen = (poll_to_next_heartbeat < poll) ? poll_to_next_heartbeat : poll;
        // 계산된 시간만큼 실제로 메시지 수신을 시도
        received = listen_message(session, poll_chosen);
        if (!received)
        {
            // 메시지를 받지 못했다면, 전체 대기 시간에서 이번에 소요된 시간을 차감
            poll -= poll_chosen;
        }
    }
    // 메시지를 받지 못했고, 아직 총 대기 시간이 남아있다면 루프를 계속 반복
